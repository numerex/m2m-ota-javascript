...
function Message(attributes) {
    ...
    if (attributes.json) attributes = JSON.parse(attributes.json,function(key, value) {
    ...
    if (attributes.buffer)
        ...
        this.majorVersion   = attributes.majorVersion === undefined ? Common.MAJOR_VERSION : attributes.majorVersion;
        this.minorVersion   = attributes.minorVersion === undefined ? Common.MINOR_VERSION : attributes.minorVersion;
        ...
        this.tuples         = attributes.tuples || [];
    }
}

Message.prototype.isAck = function(){
    return Common.isAck(this.messageType);
};

Message.prototype.isEvent = function(){
    return Common.isEvent(this.messageType);
};
...
Message.prototype.find = function(id,defaultValue){
    for (var index = 0; index < this.tuples.length; index++)
        if (this.tuples[index].id === id) return this.tuples[index].value;
    return defaultValue;
};
